...
class KafkaDeferredMessageSender implements DeferredMessageSender {
    ...
    private void send(DeferredMessage deferredMessage, Duration sendTimeout, KafkaTemplate<byte[], byte[]> kafkaTemplate) {
        ...
        try {
            ...
            if (key != null) {
                kafkaTemplate.send(topic, key, message).get(sendFutureTimeoutMillis, TimeUnit.MILLISECONDS);
            } else {
                kafkaTemplate.send(topic, message).get(sendFutureTimeoutMillis, TimeUnit.MILLISECONDS);
            }
